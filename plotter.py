...
class ADCWorker(QtCore.QObject):
    ...
    def read_adc(self):
        ...
        for channel in range(NCHANNELS):
            data["Channel {}".format(channel)] = self.adc.analog_read_volt(channel, self.data_rate)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.plot_curves = [self.plot_widget.plot() for _ in range(NCHANNELS)]
    ...
    def update_plot(self, data):
        ...
        for channel in range(NCHANNELS - 1):
            self.plot_curves[channel].setData(self.plot_data["t"], self.plot_data[f"Channel {channel}"])
        # self.plot_curves[3].setData(self.plot_data["t"], self.plot_data[f"Channel {3}"] / 10 / 1e-3)
